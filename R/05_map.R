...
# ______________________________________________________________________________
# Purpose : Compose a publication-quality map of Mecufi District river
...
# Outputs : output/mecufi_catchment_map.png  (300 dpi)
...
if (!force_rerun["map"]) {
  if (file.exists("output/mecufi_catchment_map.png")) {
    message("05_map: cache hit — skipping (set force_rerun['map'] = TRUE to regenerate)")
    stop("Map exists and force_rerun['map'] is FALSE — stopping gracefully.",
         call. = FALSE)
  }
}
...
# MAP COMPOSITION ----
# ______________________________________________________________________________
# Strategy: build base map, then conditionally append optional layers.
# This avoids NULL + tmap issues when waterways/roads layers are empty.

message("05_map: composing map...")
...
map_out <-
  ...
  ## Map furniture (layout elements — z-order independent)
  tm_graticules(
    col         = "grey70",
    alpha       = 0.4,
    lwd         = 0.3,
  ...
  tm_credits(
    text     = "Elevation: Copernicus GLO-30 DEM | Admin: OCHA | Waterways/Roads: HOT OSM | Projection: UTM Zone 37S (EPSG:32737)",
...
message("05_map: exporting to output/mecufi_catchment_map.png ...")

tmap_save(map_out,
          filename = "output/mecufi_catchment_map.png",
          width    = 250,
          height   = 200,
          units    = "mm",
          dpi      = 300)
...
message("Output: output/mecufi_catchment_map.png (300 dpi)")
